...
/*   Updated: 2018/04/01 15:44:58 by knikanor         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

void	microbs_formula(int x, int y, t_frctl *f)
{
	int		iter;
	double	a;
	double	b;
	...
	a = re_z;
	b = im_z;
	while (a * a < 70.0 && b * b < 70.0 && iter < f->iter_max)
	{
		f->tmp = a * a * a * a + b * b * b * b - 6 * a * a * b * b + 1;
		b = 4 * a * a * a * b - 4 * a * b * b * b + 1;
		a = f->tmp;
		iter++;
	}
	if (fabs(a) > 10 * f->iter_max / 50 || fabs(b) > 1000 * f->iter_max / 30)
